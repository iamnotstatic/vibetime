...
- Fish shell not supported — `vibe init` writes bash/zsh function syntax. Fish users must add the hook manually: `function Codex; vibe __wrap Codex $argv; end` (issue #1)
- `VIBE_SESSION=1` inherited by child processes that call Codex programmatically — known tradeoff of the nesting guard. A tool that spawns `Codex` as a subprocess will skip tracking for that inner invocation.
